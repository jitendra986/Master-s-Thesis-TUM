...
class DataPreprocessor:
    ...
    def _normalize_data(self):
        ...
        cols_normalize_train = self.train_df.columns.difference(['id','cycle','RUL','label1','label2'])#removing irrelevant 5 columns
        cols_normalize_validation = self.validation_df.columns.difference(['id','cycle','RUL','label1','label2'])
        
        min_max_scaler = preprocessing.MinMaxScaler()
        norm_train_df = pd.DataFrame(min_max_scaler.fit_transform(self.train_df[cols_normalize_train]),
                                     columns=cols_normalize_train,
                                     index=self.train_df.index)
        norm_validation_df = pd.DataFrame(min_max_scaler.fit_transform(self.validation_df[cols_normalize_validation]),
                                          columns=cols_normalize_validation,
                                          index=self.validation_df.index)
        
        # MinMax normalization (from 0 to 1)
        join_df_train = self.train_df[self.train_df.columns.difference(cols_normalize_train)].join(norm_train_df)
        join_df_validation = self.validation_df[self.validation_df.columns.difference(cols_normalize_validation)].join(norm_validation_df)
    ...
    def _generate_sequences(self):
        ...
        seq_gen = (list(gen_sequence(self.train_df[self.train_df['id']==id], sequence_length, sequence_cols))
                   for id in self.train_df['id'].unique())
        seq_gen_validation = (list(gen_sequence(self.validation_df[self.validation_df['id']==id], sequence_length, sequence_cols))
    ...
    def _generate_labels(self):
        ...
        self.dummy_label_array = torch.nn.functional.one_hot(data_tensor).numpy().astype(np.int64)
        self.dummy_label_array_validation = torch.nn.functional.one_hot(data_tensor_validation).numpy().astype(np.int64)